...
const Boardlist = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await deleteBoard(show)
      setBoards((prev) => prev.filter((item) => item.id !== show))
    } catch (err) {
      console.error(err)
  ...
  return (
    <Wrapper>
      <DeleteModal
        ...
        title="Are you sure? This board will be permanently deleted."
      />
      <AddBoard
        modalState={modalState}
        setModalState={(val) => !val && setModalState(undefined)}
        onSave={handleSave}
      />
      {loading ? (
